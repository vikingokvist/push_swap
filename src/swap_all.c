...
void    swap_all(t_list **stack_a, t_list **stack_b)
{
        ...
        while (stack_len(stack_a) > 2)
        {
                ...
                while (has_elements_below_midpoint(stack_a, mid_point) == 1)
                {
                        ...
                        {
                                pb(stack_a, stack_b);ft_lstprint(stack_a, stack_b);
                        }
                        ...
                        {
                                ra(stack_a);ft_lstprint(stack_a, stack_b);
                        }
                        last = get_last(stack_a);
...
//if next number isnt less, go to last node and check if its less than 7. If so rra and pb
